...
| `docs/api/student-app.md` | **worked examples by flow for the student mobile app** — generated by performing the calls, and re-verified by `tests/integration/test_api_examples.py` on every run, so it cannot drift. Regenerate with `make api-docs`. |
| `docs/design/0012-student-app-design-brief.md` | the design prompt for the student app: forty screens, each naming the endpoint behind it |
