...
FLOORS: tuple[tuple[str, int, str], ...] = (
    ...
    ("app/modules/speaking/", 100,
     "'Enforce age banding at the matching layer, not in the client.' This is "
     "that layer, and it is the only code in the product whose failure is a "
     "child-safety incident rather than a bug. `matching.py` was at 97% and one "
     "of the three unexecuted lines was the `raise UnsafePair` backstop itself — "
     "the test named for it re-implemented the check and asserted against the "
     "copy. `service.py` was at 91% and held the other end of the same invariant, "
     "the cross-band refusal on the INSERT, carrying `# pragma: no cover`."),
