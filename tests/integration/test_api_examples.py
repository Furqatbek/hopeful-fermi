...
def flow_signing_in(client, db, seed) -> list:
    ...
    # Not bound to a name any more: the refresh token used to be read out of this
    # response and passed to `/auth/refresh` below. It is in a cookie now, which
    # the recorder's client carries by itself.
    r.call(
        ...
        note=("Exchange it for a session\n"
              "The body carries only the access token — short (15 minutes) and "
              "kept in memory. The refresh token is long and rotating and is "
              "**not in the body at all**: it arrives in the `ielts_refresh` "
              "cookie, which is `HttpOnly` so no script can read it. There is "
              "nothing for the client to store and nothing for an XSS to steal "
              "beyond a token that expires in a quarter of an hour."),
        expect=(200,))
    ...
    r.call("POST", "/auth/refresh",
           note=("Rotate before the access token expires\n"
                 "**No request body.** The browser attaches the `ielts_refresh` "
                 "cookie by itself; there is nothing to send and nothing to read. "
                 "**The old token is dead the moment this succeeds.** Reusing one "
                 "is treated as theft: the server revokes every live session for "
                 "that user and answers `401 token_reuse_detected`. Serialise "
                 "refreshes — two concurrent calls from two tabs will sign the "
                 "student out of both."),
           expect=(200,))
    return r.calls
...
PREAMBLE = """
Worked examples for the **student web app**, generated by performing each flow
against a real database and recording what came back.
...
- **Auth** — `Authorization: Bearer <access token>`, held in memory only. Access
  tokens last 15 minutes. Refresh with `POST /auth/refresh`, which takes **no
  request body**: the refresh token travels in the `ielts_refresh` cookie —
  `HttpOnly`, `Secure`, `SameSite=Strict`, scoped to `/api/v1/auth` — that every
  sign-in response sets. Page script cannot read it, so an injected script steals
  at most a 15-minute access token rather than a 90-day session (ADR-0002 §6).
  Never persist the access token; never try to read the refresh token.
